...
public class MediaSessionHelper implements MediaImageCallback {
    ...
    @VisibleForTesting
    static final long NOTIFICATION_POSITION_UPDATE_MIN_INTERVAL_MILLIS = 1000;
    @VisibleForTesting static final long LOCKED_PIP_AUTO_NEXT_TRIGGER_WINDOW_MILLIS = 1200;
    @VisibleForTesting static final long LOCKED_PIP_AUTO_NEXT_ATTEMPT_COOLDOWN_MILLIS = 3000;
    ...
    private @Nullable MediaPosition mLastNotifiedMediaPosition;
    private @Nullable String mLastLockedPipAutoNextTrackKey;
    private long mLastLockedPipAutoNextAttemptElapsedRealtimeMs;
    private long mLastNotificationPositionUpdateElapsedRealtimeMs;
    ...
    private final MediaNotificationListener mControlsListener =
            new MediaNotificationListener() {
                ...
                public void onPlay(int actionSource) {
                    ...
                    if (maybeDispatchBraveYouTubeCommand("play")) return;
                ...
                public void onPause(int actionSource) {
                    ...
                    if (maybeDispatchBraveYouTubeCommand("pause")) return;
                ...
                public void onMediaSessionAction(int action) {
                    ...
                    if (action == MediaSessionAction.NEXT_TRACK
                            && maybeDispatchBraveYouTubeCommand("next")) {
                        return;
                    }
                    if (action == MediaSessionAction.PREVIOUS_TRACK
                            && maybeDispatchBraveYouTubeCommand("previous")) {
                        return;
                    }
                    if (action == MediaSessionAction.SEEK_FORWARD
                            && maybeDispatchBraveYouTubeSeekBy(10)) {
                        return;
                    }
                    if (action == MediaSessionAction.SEEK_BACKWARD
                            && maybeDispatchBraveYouTubeSeekBy(-10)) {
                        return;
                    }
    ...
    private boolean maybeDispatchBraveYouTubeCommand(String methodName) {
        if (mWebContents == null) return false;

        try {
            Class<?> helperClass =
                    Class.forName(
                            "org.chromium.chrome.browser.youtube_script_injector.BraveYouTubeScriptInjectorNativeHelper");
            java.lang.reflect.Method method =
                    helperClass.getMethod(methodName, WebContents.class);
            Object result = method.invoke(null, mWebContents);
    ...
    private boolean maybeDispatchBraveYouTubeSeekBy(int offsetSeconds) {
        if (mWebContents == null) return false;
        ...
        try {
            Class<?> helperClass =
                    Class.forName(
                            "org.chromium.chrome.browser.youtube_script_injector.BraveYouTubeScriptInjectorNativeHelper");
            java.lang.reflect.Method method =
                    helperClass.getMethod("seekBy", WebContents.class, int.class);
            Object result = method.invoke(null, mWebContents, offsetSeconds);
            return result instanceof Boolean && (Boolean) result;
        } catch (ReflectiveOperationException | RuntimeException e) {
            return false;
        }
    ...
    private void hideNotificationInternal() {
        ...
        resetNotificationPositionThrottle();
    ...
    private MediaSessionObserver createMediaSessionObserver(MediaSession mediaSession) {
        return new MediaSessionObserver(mediaSession) {
            ...
            public void mediaSessionDestroyed() {
                resetLockedPipAutoNextTracking();
                hideNotificationImmediately();
            ...
            public void mediaSessionPositionChanged(@Nullable MediaPosition position) {
                ...
                if (position == null) {
                    ...
                    maybeDispatchLockedPipAutoNext(position);
                }
    ...
    private void resetNotificationPositionThrottle() {
        mLastNotifiedMediaPosition = null;
        mLastNotificationPositionUpdateElapsedRealtimeMs = 0;
    }
    ...
    private boolean isActivityInPictureInPictureMode() {
    ...
    private void resetLockedPipAutoNextTracking() {
        mLastLockedPipAutoNextTrackKey = null;
        mLastLockedPipAutoNextAttemptElapsedRealtimeMs = 0L;
    }

    private static long estimateCurrentMediaPositionMillis(MediaPosition position) {
        long estimatedPosition = position.getPosition();
        if (position.getPlaybackRate() > 0f) {
            long elapsedSinceUpdateMs =
                    Math.max(0L, SystemClock.elapsedRealtime() - position.getLastUpdatedTime());
            estimatedPosition += (long) (elapsedSinceUpdateMs * position.getPlaybackRate());
        }
        return Math.max(0L, Math.min(position.getDuration(), estimatedPosition));
    }

    private String buildLockedPipAutoNextTrackKey(MediaMetadata metadata, MediaPosition position) {
        return String.join(
                "|",
                String.valueOf(metadata.getTitle()),
                String.valueOf(metadata.getArtist()),
                String.valueOf(metadata.getAlbum()),
                String.valueOf(position.getDuration()));
    }

    private void maybeDispatchLockedPipAutoNext(MediaPosition position) {
        if (mWebContents == null) return;
        if (!isDeviceLocked() || !isActivityInPictureInPictureMode()) return;
        if (mWebContents.getVisibility() == Visibility.VISIBLE) return;
        if (!mMediaSessionActions.contains(MediaSessionAction.NEXT_TRACK)) return;
        if (position.getDuration() <= 0 || position.getPlaybackRate() <= 0f) return;

        long estimatedPositionMs = estimateCurrentMediaPositionMillis(position);
        long remainingDurationMs = position.getDuration() - estimatedPositionMs;
        if (remainingDurationMs <= 0
                || remainingDurationMs > LOCKED_PIP_AUTO_NEXT_TRIGGER_WINDOW_MILLIS) {
            return;
        }

        String trackKey = buildLockedPipAutoNextTrackKey(getMetadata(), position);
        long now = SystemClock.elapsedRealtime();
        if (TextUtils.equals(trackKey, mLastLockedPipAutoNextTrackKey)
                && now - mLastLockedPipAutoNextAttemptElapsedRealtimeMs
                        < LOCKED_PIP_AUTO_NEXT_ATTEMPT_COOLDOWN_MILLIS) {
            return;
        }

        mLastLockedPipAutoNextTrackKey = trackKey;
        mLastLockedPipAutoNextAttemptElapsedRealtimeMs = now;

        boolean dispatched = maybeDispatchBraveYouTubeCommand("next");
        Log.i(
                TAG,
                "OTB_PIP event=locked_pip_auto_next_from_position dispatched=%b remaining_ms=%d visibility=%d",
                dispatched,
                remainingDurationMs,
                mWebContents.getVisibility());
    }
    ...
    private void updateNotificationPosition() {
        ...
        if (mMediaPosition == null) {
            resetNotificationPositionThrottle();
            mNotificationInfoBuilder.setMediaPosition(null);
            showNotification();
            return;
        }
        if (shouldThrottleNotificationPositionUpdate(mMediaPosition)) return;

        mNotificationInfoBuilder.setMediaPosition(mMediaPosition);
        mLastNotifiedMediaPosition = mMediaPosition;
        mLastNotificationPositionUpdateElapsedRealtimeMs = SystemClock.elapsedRealtime();
        showNotification();
    }

    private boolean shouldThrottleNotificationPositionUpdate(MediaPosition position) {
        if (mLastNotifiedMediaPosition == null) return false;

        if (position.getDuration() != mLastNotifiedMediaPosition.getDuration()) {
            return false;
        }

        if (position.getPlaybackRate() != mLastNotifiedMediaPosition.getPlaybackRate()) {
            return false;
        }

        long positionDelta =
                Math.abs(position.getPosition() - mLastNotifiedMediaPosition.getPosition());
        if (positionDelta >= NOTIFICATION_POSITION_UPDATE_MIN_INTERVAL_MILLIS) {
            return false;
        }

        long now = SystemClock.elapsedRealtime();
        return now - mLastNotificationPositionUpdateElapsedRealtimeMs
                < NOTIFICATION_POSITION_UPDATE_MIN_INTERVAL_MILLIS;
    }
