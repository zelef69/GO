...
class AdBlockSubscriptionServiceManager {
 ...
 private:
  ...
  void StartFallbackDownload(const GURL& sub_url);
  void OnFallbackDownloadDirectoryReady(const GURL& sub_url, bool created);
  void OnFallbackDownloadComplete(const GURL& sub_url,
                                  base::FilePath temporary_file);
  void OnFallbackDownloadReplaced(const GURL& sub_url, bool success);
  AdBlockSubscriptionFiltersProvider* FindAnyEnabledSubscriptionProvider(
      bool is_default_engine);
  void ScheduleSubscriptionEngineReload(bool is_default_engine);
  void FlushPendingSubscriptionEngineReloads();

  bool initialized_;
  void LoadSubscriptionServices();
  void EnsureBuiltInSubscriptions();
  ...
  CreateSubscriptionProvider(const SubscriptionInfo& info);
  ...
  std::map<GURL, std::unique_ptr<network::SimpleURLLoader>>
      fallback_downloaders_ GUARDED_BY_CONTEXT(sequence_checker_);
  bool pending_default_engine_reload_ GUARDED_BY_CONTEXT(sequence_checker_) =
      false;
  bool pending_additional_engine_reload_ GUARDED_BY_CONTEXT(
      sequence_checker_) = false;
  base::OneShotTimer pending_subscription_reload_timer_
      GUARDED_BY_CONTEXT(sequence_checker_);
