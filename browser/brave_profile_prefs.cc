...
void OverrideDefaultPrefValues(user_prefs::PrefRegistrySyncable* registry) {
  ...
  registry->SetDefaultPrefValue(
      prefs::kEnableMediaRouter, base::Value(!BUILDFLAG(IS_ONETABYT)));
...
// Private New Tab Page
  ...
  // OneTabTube prunes the NTP-sponsored surface, but startup metrics code still
  // reads these prefs. Keep the prefs registered so startup stays stable even
  // when the UI path is not exposed.
  ...
  registry->RegisterBooleanPref(kEnableMediaRouterOnRestart,
                                !BUILDFLAG(IS_ONETABYT));
