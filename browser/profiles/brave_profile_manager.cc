...
void LogOneTabTubeDefaultPrefs(Profile* profile) {
  auto* map = HostContentSettingsMapFactory::GetForProfile(profile);
  auto* prefs = profile->GetPrefs();
  if (!map || !prefs) {
    return;
  ...
  LOG(INFO) << "OTB_ADBLOCK event=default_prefs profile_regular="
            << profile->IsRegularProfile() << " shields="
            << brave_shields::GetBraveShieldsEnabled(map, GURL()) << " ad="
            << brave_shields::ControlTypeToString(
            ...
            << " preload_state="
            << static_cast<int>(prefetch::GetPreloadPagesState(*prefs))
            << " network_prediction="
...
// Chromecast is enabled by default on Android.
  ...
  if (profile->IsRegularProfile()) {
    ...
    if (prefs &&
      ...
      LOG(INFO) << "OTB_PERF event=profile_preload_state enforced=extended";
    }

    static bool did_request_onetabtube_adblock_components = false;
    if (!did_request_onetabtube_adblock_components) {
      did_request_onetabtube_adblock_components = true;
      auto* on_demand_updater =
          brave_component_updater::BraveOnDemandUpdater::GetInstance();
      on_demand_updater->EnsureInstalled(
          brave_shields::kAdBlockResourceComponentId);
      on_demand_updater->OnDemandUpdate(
          brave_shields::kAdBlockResourceComponentId,
          component_updater::OnDemandUpdater::Priority::FOREGROUND);
      on_demand_updater->EnsureInstalled(
          brave_shields::kAdBlockFilterListCatalogComponentId);
      on_demand_updater->OnDemandUpdate(
          brave_shields::kAdBlockFilterListCatalogComponentId,
          component_updater::OnDemandUpdater::Priority::FOREGROUND);
      LOG(INFO) << "OTB_ADBLOCK event=startup phase=ProfileInit "
                << "resource_component="
                << brave_shields::kAdBlockResourceComponentId
                << " catalog_component="
                << brave_shields::kAdBlockFilterListCatalogComponentId;
...
#if BUILDFLAG(IS_ANDROID) && BUILDFLAG(IS_ONETABYT)
  if (profile->IsRegularProfile()) {
    LogOneTabTubeDefaultPrefs(profile);
  }
#endif
