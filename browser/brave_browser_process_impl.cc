...
void BraveBrowserProcessImpl::StartBraveServices() {
  ...
  // OneTabTube depends on the standard Brave Shields/adblock stack. Warm it
  // during startup so component-backed resources and lists begin registering
  // before the first YouTube navigation.
  auto* on_demand_updater =
      brave_component_updater::BraveOnDemandUpdater::GetInstance();
  LOG(INFO) << "OTB_ADBLOCK event=startup phase=StartBraveServices "
            << "local_data_files_started=1 onetabyt=1";
  ad_block_service();
  on_demand_updater->EnsureInstalled(brave_shields::kAdBlockResourceComponentId);
  ...
  on_demand_updater->OnDemandUpdate(
      brave_shields::kAdBlockFilterListCatalogComponentId,
      component_updater::OnDemandUpdater::Priority::FOREGROUND);
  LOG(INFO) << "OTB_ADBLOCK event=startup phase=WarmAdBlockComponents "
            << "resource_component="
            << brave_shields::kAdBlockResourceComponentId
            << " catalog_component="
            << brave_shields::kAdBlockFilterListCatalogComponentId;
...
brave_shields::AdBlockService* BraveBrowserProcessImpl::ad_block_service() {
  if (!ad_block_service_) {
    ...
    ad_block_service_ = std::make_unique<brave_shields::AdBlockService>(
        ...
        shared_url_loader_factory(), AdBlockSubscriptionDownloadManagerGetter(),
        profile_manager()->user_data_dir().Append(
            profile_manager()->GetInitialProfileDir()));
    LOG(INFO) << "OTB_ADBLOCK event=service_constructed source=browser_process";
  }
