...
public abstract class BraveMainPreferencesBase extends BravePreferenceFragment
    ...
    private interface AccountControllerHandle {
        void destroy();
        void updateUI();
    }
    ...
    private static final String PREF_BRAVE_ACCOUNT_USER_INFO = "user_info";
    private static final String PREF_BRAVE_ACCOUNT_SIGN_OUT = "sign_out";
    private static final String PREF_BRAVE_ACCOUNT_ALMOST_THERE = "almost_there";
    private static final String PREF_BRAVE_ACCOUNT_RESEND_CONFIRMATION_EMAIL =
            "resend_confirmation_email";
    private static final String PREF_BRAVE_ACCOUNT_CANCEL_REGISTRATION = "cancel_registration";
    private static final String PREF_BRAVE_ACCOUNT_GET_STARTED = "get_started";
    private static final String[] BRAVE_ACCOUNT_PREFERENCE_KEYS =
            new String[] {
                PREF_BRAVE_ACCOUNT_SECTION,
                PREF_BRAVE_ACCOUNT_USER_INFO,
                PREF_BRAVE_ACCOUNT_SIGN_OUT,
                PREF_BRAVE_ACCOUNT_ALMOST_THERE,
                PREF_BRAVE_ACCOUNT_RESEND_CONFIRMATION_EMAIL,
                PREF_BRAVE_ACCOUNT_CANCEL_REGISTRATION,
                PREF_BRAVE_ACCOUNT_GET_STARTED
            };

    private final HashMap<String, Preference> mRemovedPreferences = new HashMap<>();
    private @Nullable AccountControllerHandle mAccountController;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mAccountController = maybeCreateAccountController();
    ...
    private void updateBravePreferences() {
        ...
        if (OneTabYouTubeMode.isEnabled()) {
            ...
            removePreferenceIfPresent(PREF_USE_CUSTOM_TABS);
            removePreferenceIfPresent(PREF_BRAVE_LANGUAGES);
            ...
            removePreferenceIfPresent(PREF_AUTOFILL_PRIVATE_WINDOW);
        }
    ...
    private void rearrangePreferenceOrders() {
        ...
        for (String key : BRAVE_ACCOUNT_PREFERENCE_KEYS) {
            if (mAccountController != null) {
        ...
        rateBravePreference.setOnPreferenceClickListener(
                new Preference.OnPreferenceClickListener() {
                    ...
                    public boolean onPreferenceClick(Preference preference) {
                        ...
                        BraveRateDialogLauncher.show(getParentFragmentManager(), true);
                        return true;
                    }
                });
    ...
    private @Nullable AccountControllerHandle maybeCreateAccountController() {
        if (OneTabYouTubeMode.isEnabled()) {
            return null;
        }
        ...
        try {
            Class<?> controllerClass =
                    Class.forName("org.chromium.chrome.browser.settings.BraveAccountSectionController");
            Object controller =
                    controllerClass
                            .getMethod(
                                    "maybeCreate", PreferenceFragmentCompat.class, Profile.class)
                            .invoke(null, this, getProfile());
            if (controller == null) {
                return null;
            }

            return new AccountControllerHandle() {
                @Override
                public void destroy() {
                    invokeControllerMethod(controller, "destroy");
                }

                @Override
                public void updateUI() {
                    invokeControllerMethod(controller, "updateUI");
                }
            };
    ...
    private static void invokeControllerMethod(Object controller, String methodName) {
        try {
            controller.getClass().getMethod(methodName).invoke(controller);
        } catch (ReflectiveOperationException e) {
            throw new IllegalStateException(
                    "Failed to call Brave account controller method: " + methodName, e);
        }
    }
}
