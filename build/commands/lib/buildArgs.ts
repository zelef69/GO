...
export function getBuildArgs(config: typeof Config) {
  ...
  if (config.targetOS === 'android') {
    args.is_onetabyt = true
    if (!args.updater_dev_endpoint) {
      args.updater_dev_endpoint =
        'https://go-updater.brave.com/service/update2/json'
    }
    if (!args.updater_prod_endpoint) {
      args.updater_prod_endpoint =
    ...
    if (!config.isReleaseBuild()) {
      ...
      args.chrome_public_manifest_package = 'com.onetabtube.browser_default'
    } else if (config.channel === '') {
      args.android_channel = 'stable'
      args.chrome_public_manifest_package = 'com.onetabtube.browser'
    } else if (config.channel === 'beta') {
      args.chrome_public_manifest_package = 'com.onetabtube.browser_beta'
    } else if (config.channel === 'dev') {
      args.chrome_public_manifest_package = 'com.onetabtube.browser_dev'
    } else if (config.channel === 'nightly') {
      args.android_channel = 'canary'
      args.chrome_public_manifest_package = 'com.onetabtube.browser_nightly'
    }
    ...
    args.brave_android_public_apk_name = 'OneTabTube'
    args.brave_android_public_bundle_name = 'OneTabTube'
