...
const getAndroidOutputFormat = () => {
  return config.targetAndroidOutputFormat || 'aab'
}

const getAndroidBuildTarget = () => {
  return getAndroidOutputFormat() === 'aab'
    ? 'chrome_public_bundle'
    : 'chrome_public_apk'
}

const getAndroidArtifactPath = () => {
  const extension = getAndroidOutputFormat()
  return path.join(config.outputDir, 'apks', `ChromePublic.${extension}`)
}
...
const chromiumConfigs = {
  ...
  'android': {
    buildTargets: [getAndroidBuildTarget()],
    processArtifacts: () => {
      fs.moveSync(
        getAndroidArtifactPath(),
        path.join(
          config.outputDir,
          `${getOutputFilename()}.${getAndroidOutputFormat()}`,
        ),
      )
    },
  },
...
function getChromiumGnArgs() {
  ...
  if (targetOs === 'android') {
    ...
    args.target_android_output_format = getAndroidOutputFormat()
  } else {
